// Pure Express app — no startup side-effects (migration, listen, retry job).
// Imported by server.js for production and by tests directly.
...
// ── Products ──────────────────────────────────────────────────────────────────
app.post('/api/companies/:companyId/products', verifyToken, requireCompanyAccess, validate('createProduct'), productController.createProduct);
app.get('/api/companies/:companyId/products', verifyToken, requireCompanyAccess, productController.listProducts);
...
app.put('/api/companies/:companyId', verifyToken, requireCompanyAccess, validate('updateCompany'), async (req, res, next) => {
  try {
...
// ── Quotations ────────────────────────────────────────────────────────────────
app.post('/api/companies/:companyId/quotations', verifyToken, requireCompanyAccess, validate('createQuotation'), quotationController.createQuotation);
app.get('/api/companies/:companyId/quotations', verifyToken, requireCompanyAccess, quotationController.listQuotations);
