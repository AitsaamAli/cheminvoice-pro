...
class PDFService {
  async generateInvoicePDF(invoice) {
    return new Promise((resolve, reject) => {
      ...
      // Header
      doc.fontSize(22).font('Helvetica-Bold').text('Nizaam.com', { align: 'center' });
      doc.fontSize(9).font('Helvetica').fillColor('#444444')
        .text('FBR Digital Invoicing — Pakistan', { align: 'center' });
      doc.moveDown(0.5);

      // Company Info
      doc.fillColor('#000000').fontSize(14).font('Helvetica-Bold').text(invoice.sellerBusinessName);
      ...
      if (invoice.referenceInvoiceNo) {
        doc.text(`Reference: ${invoice.referenceInvoiceNo}`, 40);
      }
      ...
      // Show further tax if applicable (unregistered buyer — SN002)
      const furtherTax = parseFloat(invoice.totalFurtherTax || 0);
      if (furtherTax > 0) {
        doc.text(`Further Tax (3% — Unregistered Buyer): PKR ${furtherTax.toLocaleString('en-PK', { maximumFractionDigits: 0 })}`, { align: 'right' });
      }
      ...
      doc.fontSize(8).font('Helvetica').fillColor('#444444')
        .text(`FBR Status: ${invoice.fbrStatus || 'PENDING'}   |   SRO 1413(I)/2025`, { align: 'center' });
      if (invoice.fbrInvoiceNumber) {
        doc.text(`FBR Invoice No (IRN): ${invoice.fbrInvoiceNumber}`, { align: 'center' });
      }

      doc.moveDown(1);
      doc.fontSize(7).fillColor('#888888')
        .text('Computer-generated invoice. No signature required.', { align: 'center' })
        .text('Nizaam.com — Pakistan\'s FBR Digital Invoicing Platform', { align: 'center' });
